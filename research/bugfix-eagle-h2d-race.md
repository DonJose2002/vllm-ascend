...
>
> **机制候选重开 → ✅ 已定谳(2026-08-28 第八轮,staged-copy 判别实验)**:
> **(b) 确认**——复活竞态下单页照崩(ok=0/8),换 8 页私有快照页照绿
> (ok=8/8,计数 `c2=7 c3=0 esc=none` vs 单页时代 `c2=7 c3=1 esc=[-1,-1]`,
> 唯一变量=拷贝源页):-1 经"晚执行的异步拷贝读到 host 已重写的单页源"
> 进入,流序与可见性全程无辜;(a) torch_npu 提交序与 (c) 图重放均出局
> (私有页救不了它们)。判别实验与全部数据见归因 §14 第七/八轮。
>
...
>
> ⚠️ **2026-08-28 增补 → ✅ 当日定谳**:计数器读数全部有效;"拷贝未落地 →
> where 读设备侧旧值"(机制 (a) 形态)与"拷贝读到 host 已改写的 -1 并亲自
> 送达"(机制 (b))起初均与读数相容,当日 staged-copy 判别实验(私有快照页
> vs 单页,单变量)裁决:**(b) 确认,(a) 出局**——详见文首横幅与归因 §14。
...
**Run 4 定谳(2026-08-25,NPU2,commit `7ba620b9c`)**:读数 `steps=843 c1=837 c2=7 c3=1 c1x=0 esc=[-1,-1]`——

- **esc=[-1,-1]**:逃逸值就是 stale 的 -1 哨兵,哨兵故事定谳;
- **c1x=0**:排除哨兵后 buffer 里零真垃圾——0xa5a5 从未进入值路径(SDMA 撕裂落地机制正式出局;plog args[18] 的 0xa5a5 是 kernel 参数区其他字段,与索引值无涉);
- **数字逐位复现 Run 2'**(843/837/7/1):bench 确定性下逃逸点固定在同一 step——**"16K 必现"从概率堆积升级为确定性事件**(该工作负载下逃逸点必然到达;"4K 偶发"则对应不同负载的逃逸点可能落在门关步或未到达)。
...
**Run 4 结果(同日,NPU2)**:绿跑 8/8(accept 2.4945),`steps=843 c1=837 c2=7 c3=1 c1x=0 esc=[-1,-1]` → **哨兵故事定谳**(第一行);数值与 Run 2' 逐位复现。预期解读注兑现:c1x=0(buffer 只有哨兵与真值);§2.7.1 的修正机制就此关闭全部开放点。
