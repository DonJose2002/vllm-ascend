...
- **staged vs 事件协议的本质差异(2026-09-11 会话澄清入档)**:staged
  只是把覆写**推迟** N 步——概率界,安全条件 = N 步 > 拷贝最大在途
  寿命(≈2048 环深/每步任务数 + SDMA 瞬态积压,负载依赖、无硬上界;
  run D 矩阵"D2 崩 → N=8 不安全 → 加深 64 再判"格即此预设),N=8
  全绿只证明**该负载**下在途寿命 <8 步;事件协议是**构造保证**——
  改写前 `event.synchronize()`,无论拷贝滞后多少步,覆写在途拷贝在
  构造上不可能发生。两者同速(25.9≈25.8)时,构造保证 + 无 N 调参
  + 无页环管理构成选型的完整理由,不只是"更简"。
