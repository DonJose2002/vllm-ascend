...
> **缺陷机制已于当日定谳**(staged-copy 判别实验:单页崩 0/8 vs 私有快照页
> 绿 8/8 且 `c2=7 c3=0 esc=none` vs 单页 `c3=1 esc=[-1,-1]`——-1 经"晚执行
> 拷贝读 host 已重写的单页源"进入,use-after-rewrite,非任何排序/可见性
> 问题;(a)/(c) 出局),完整定谳见归因 §14 第七/八轮。
